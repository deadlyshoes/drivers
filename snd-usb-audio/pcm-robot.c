			       ...
			       struct urb *urb)
{
    ...
    char input_buffer[128], output_buffer[128];
    int j, k, input_pointer = 0;
	...
	for (i = 0; i < urb->number_of_packets; i++) {
        ...
        for (j = 0; j < cp; j++){
            input_buffer[input_pointer] = cp[j];
            input_pointer = (input_pointer + 1) % 128;
